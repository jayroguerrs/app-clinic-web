...
@Component({
    selector: 'app-mdl-comprobante-tipo-nota-debito',
    templateUrl: 'mdl-comprobante-tipo-nota-debito.component.html'
})
export class MdlComprobanteTipoNotaDebitoComponent implements OnInit, OnDestroy {

    @Input() data: ComprobanteTipoNotaDebito | null = null;
    ...
    onSubmit(): void {
        ...
        if (this.data ){
            // EDITAR
          Swal.fire({
            html: `Desea editar los datos del tipo de nota de débito <b>${this.data?.nombre}</b>??`,
            icon: 'question',
              ...
              if(result.isConfirmed) {
                ...
                this.api.modificar(this.model).subscribe((res: boolean | ErrorSistema)=> {
                    ...
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        html: `Se modifico los datos del tipo de nota de débito!!!`,
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: 'Aceptar',
                        customClass: {
                  ...
                  error => {
                    Swal.fire({
                      title: 'Error',
                      html: `Error al intentar modificar los datos del tipo de nota de débito`,
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
                    ...
                    console.log('Error al modificar el tipo de nota de débito', error);
                    this.loading = false;
                  });
              }else{
                this.cerrarModal();
            ...
            // NUEVO
          Swal.fire({
            title: 'Desea registrar el nuevo tipo de nota de débito??',
            icon: 'question',
            allowOutsideClick: false,
            allowEscapeKey: false,
            buttonsStyling: false,
              ...
              if(result.isConfirmed) {
                ...
                this.api.registrar(this.model).subscribe((res: boolean | ErrorSistema) => {
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        text: "Se registro el tipo de nota de débito con exito",
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: 'Aceptar',
                        customClass: {
                  ...
                  error => {
                    this.loading = false;
                    console.log('Error al registrar el tipo de nota de débito', error);
                    Swal.fire({
                      title: 'Error',
                      text: 'Error al registrar el tipo de nota de débito',
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
