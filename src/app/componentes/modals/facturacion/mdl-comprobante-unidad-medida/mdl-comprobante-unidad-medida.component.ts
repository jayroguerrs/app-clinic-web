...
@Component({
    selector: 'app-mdl-comprobante-unidad-medida',
    templateUrl: 'mdl-comprobante-unidad-medida.component.html'
})
export class MdlComprobanteUnidadMedidaComponent implements OnInit, OnDestroy {

    @Input() data: ComprobanteUnidadMedida | null = null;
    ...
    onSubmit(): void {
        ...
        if (this.data ){
            // EDITAR
          Swal.fire({
            html: `Desea editar los datos de <b>${this.data?.nombre}</b>??`,
            icon: 'question',
              ...
              if(result.isConfirmed) {
                ...
                this.api.modificar(this.model).subscribe((res: boolean | ErrorSistema)=> {
                  ...
                  error => {
                    Swal.fire({
                      title: 'Error',
                      html: `Error al intentar modificar los datos`,
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
            ...
            // NUEVO
          Swal.fire({
            title: 'Desea registrar la nueva unidad de medida??',
            icon: 'question',
            allowOutsideClick: false,
            allowEscapeKey: false,
            buttonsStyling: false,
              ...
              if(result.isConfirmed) {
                ...
                this.api.registrar(this.model).subscribe((res: boolean | ErrorSistema) => {
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        text: "Se registro la unidad de medida con exito",
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: 'Aceptar',
                        customClass: {
