...
@Component({
    selector: 'app-mdl-factura-moneda',
    templateUrl: 'mdl-factura-moneda.component.html'
})
export class MdlFacturaMonedaComponent implements OnInit, OnDestroy {

    @Input() data: FacturaMoneda | null = null;
    ...
    initForm(): void {
        this.frmGroup = this.formBuilder.group({
          simbolo : new FormControl(null, [Validators.required, Validators.maxLength(4)]),
    ...
    onSubmit(): void {
        ...
        if (this.data ){
            // EDITAR
          Swal.fire({
            html: `Desea editar los datos del tipo de moneda <b>${this.data?.simbolo}</b>??`,
            icon: 'question',
              ...
              if(result.isConfirmed) {
                ...
                this.api.modificar(this.model).subscribe((res: boolean | ErrorSistema)=> {
                    ...
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        html: `Se modifico los datos del tipo de moneda!!!`,
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: 'Aceptar',
                        customClass: {
                  ...
                  error => {
                    Swal.fire({
                      title: 'Error',
                      html: `Error al intentar modificar los datos del tipo de moneda`,
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
                    ...
                    console.log('Error al modificar el tipo de moneda', error);
                    this.loading = false;
                  });
              }else{
                this.cerrarModal();
            ...
            // NUEVO
          Swal.fire({
            title: 'Desea registrar el nuevo tipo de moneda??',
            icon: 'question',
            allowOutsideClick: false,
            allowEscapeKey: false,
            buttonsStyling: false,
              ...
              if(result.isConfirmed) {
                ...
                this.api.registrar(this.model).subscribe((res: boolean | ErrorSistema) => {
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        text: "Se registro el tipo de moneda con exito",
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: 'Aceptar',
                        customClass: {
                  ...
                  error => {
                    this.loading = false;
                    console.log('Error al registrar el tipo de moneda', error);
                    Swal.fire({
                      title: 'Error',
                      text: 'Error al registrar el tipo de moneda',
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
