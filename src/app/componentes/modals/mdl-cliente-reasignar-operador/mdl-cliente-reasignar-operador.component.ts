...
@Component({
  selector: 'app-mdl-cliente-reasignar-operador',
  templateUrl: './mdl-cliente-reasignar-operador.component.html',
  styleUrls: ['./mdl-cliente-reasignar-operador.component.scss'],
  providers: [DatePipe]
})
export class MdlClienteReasignarOperadorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  inicializarFormulario() : void {
    ...
    this.frmOperadores = this.formBuilder.group({
      ...
      idModo: new FormControl(1, Validators.required)
    });

    this.frmOperadores.get('fechaConfirmacion').valueChanges.subscribe((res) => {
      if(res){
        this._clienteAsignados.forEach(item => {
          item.fechaConfirmacion = res;
        });
      }
    });

    this.frmOperadores.get('idModo').valueChanges.subscribe((res) => {
      if(res){
        this._clienteAsignados.forEach(item => {
          item.idTipo = parseInt(res, 10);
        });
      }
    });
  ...
  buildtableCliente(): void{
    this.dtResponsivePreferentes = {
      ajax: (_dataTablesParameters: any, callback) => {
        ...
        this.spinner.show();
        if(this.reloadBD){
          const subs = this.clienteAsignadoService.obtenerParaReasignados(this.filtro.fechaConfirmacion, this.filtro.tipoCliente, this.filtro.tipoSiguiente, this.filtro.idSede, this.filtro.asignadoA, this.filtro.asignadoPor).subscribe(
      ...
      columns: [
        ...
        { title : 'CLIENTES', data: 'nombres' ,
          render: (data, type, row) => {
            return `<span style="font-size:11px;">${row.nombres.toUpperCase()} ${row.apellidos.toUpperCase()}</span>`; }
        },
        ...
        { title: 'T. CLIENTE',   data: 'tipoCliente', width: "4%"},
        {
          ...
          render: (data, type, row) => {
            return `<span class="rounded px-2 text-white fs-11px" style="background-color: ${row.estadoColor}">${row.estado}</span>`;
          }
        }
      ],
      createdRow: (row: any | Node, data: any | Object, index: number) => {
        ...
        // ------------------------------------------------
        if(this._clientesSeleccionados.findIndex( x => x.id === data.id) >= 0 ){
        ...
        $(row).find('input.cita-select').on('change',(evt) => {
          if($(evt.target).is(':checked')){
            ...
            $('td:eq(0)', row).find('.index').text(this._clientesSeleccionados.findIndex( x => x.id === data.id) + 1 );
            ...
            const _this = this;
            table.rows({ selected: true }).every( function ( rowIdx, tableLoop, rowLoop ) {
              $(this.node()).find('.index').text(_this._clientesSeleccionados.findIndex( x => x.id === this.data().id) + 1);
            });
          }else{
            this._clientesSeleccionados = this._clientesSeleccionados.filter( x => x.id !== data.id);
            $('td:eq(0)', row).find('.index').text('');
      ...
      rowCallback: (row: Node, data: any | Object, index: number) => {
        ...
        $('td', row).on('click', () => {
          ...
          this.numClientesSeleccionadas = this.clientesSeleccionados.length;
          this.maestroClienteAsignado.find(x => x.id == data.id).seleccionado = true;
  ...
  distribuir(): void {
    ...
    if(!this.frmOperadores.controls.fechaConfirmacion.value){
      this.utilsService.mostrarToast('Seleccione la fecha de confirmación', 'warning');
      return;
    }
      ...
      this._usuariosSeleccionados.forEach( u => {
        ...
        for (let i = posicion_ini; i <= posicion_fin; i++) {
          ...
          this._clienteAsignados.push({
            id: this._clientesSeleccionados[i].id,
        ...
        if( cont_residuo < residuo ){
          ...
          this._clienteAsignados.push({
            ...
            id: this._clientesSeleccionados[posicion_ini].id,
    ...
    for(var  i = 0 ; i < this._clientesSeleccionados.length; i++) {
      ...
      const clienteAsig = {
        id: parseInt(this._clientesSeleccionados[i].id, 10),
    ...
    this.clienteAsignados.forEach(element => {
      const id = element.id;
      this.maestroClienteAsignado.find(u => u.id == id).asignado = element.usuario ;
    });
  ...
  grabarDistribuirClientes(): void {
    ...
    const subs = this.clienteAsignadoService.reasignarLista(model).subscribe(
      resultado => {
        this.utilsService.mostrarToast('Los clientes fueron reasignados correctamente', 'success');
        this.OnSaved.emit(true);
        this.modal.close(true);
      },
      error => {
        console.log(error);
        this.utilsService.mostrarToast('Ocurrio un error al intentar reasignar los clientes', 'error');
      }
    );
    this.subscriptions.push(subs);
  ...
  obtenerUsuarioAsignado(): void{
    const subs = this.usuarioService.usuarioObtenerById(this.filtro.asignadoA).subscribe((res) => {
      this.usuario = res.nombre;
    }, (error) => {
      this.utilsService.mostrarToast('Ocurrio un error al obtener el usuario asignado', 'error');
      console.log(error);
    });
    this.subscriptions.push(subs);
  }
