...
@Component({
  selector: 'mdl-cita-abandonada-en-espera-asignacion-operador',
  templateUrl: './mdl-cita-abandonada-en-espera-asignacion-operador.component.html',
  styleUrls: ['./mdl-cita-abandonada-en-espera-asignacion-operador.component.scss'],
  providers: [DatePipe]
})
export class MdlCitaAbandonadaEnEsperaAsignacionOperadorComponent implements OnInit, OnDestroy {
      ...
      ajax: (_dataTablesParameters: any, callback) => {
        ...
        if(this.reloadBD){
            if(this.idUsuarioReasignacion == 0) {
              ...
              this.sbcAsignadaObtener = this.citaAsignadaService.obtenerAbandonadosEnEspera(this.frmOperadores.controls.fechaCita.value).subscribe(
                (res: any) => {
  ...
  distribuirCitas(): void {
      ...
      this._usuariosSeleccionados.forEach( u => {
        ...
        for (let i = posicion_ini; i <= posicion_fin; i++) {
          ...
          this._citasAsignadas.push({
            idCitaAsignacion: this._citasSeleccionadas[i].id,
            idCita: this._citasSeleccionadas[i].idCita,
            idUsuarioOperador: u.idUsuario,
            fechaConfirmacion:  this._citasSeleccionadas[i].fechaConfirmacion,
        ...
        if( cont_residuo < residuo ){
          ...
          this._citasAsignadas.push({
            ...
            idUsuarioOperador: u.idUsuario,
            fechaConfirmacion:  this._citasSeleccionadas[posicion_ini].fechaConfirmacion,
            idUsuarioRegistra: this.usuarioActual.idUsuario,
            usuario: u.usuario
          });
          cont_residuo++;
    ...
    for(var  i = 0 ; i < this._citasSeleccionadas.length; i++) {
      ...
      const citaAsig = {
        idCitaAsignacion: parseInt(this._citasSeleccionadas[i].id, 10),
  ...
  grabarDistribuirCitas(): void {
    ...
    this.sbcGuardarCitaAsignada = this.citaAsignadaService.asignarAbandonadosEnEspera(model).subscribe(
      resultado => {
        this.utilsService.mostrarToast('Citas abandonadas en espera asignadas correctamente', 'success');
        this.modal.close();
      },
