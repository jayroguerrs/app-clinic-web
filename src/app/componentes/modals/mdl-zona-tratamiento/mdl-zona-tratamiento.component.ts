...
import {ZonaTratamiento} from "../../../shared/models/zonas";
import {ZonaTratamientoService} from "../../../shared/services/zona-tratamiento.service";

@Component({
    selector: 'app-mdl-zona-tratamiento',
    templateUrl: 'mdl-zona-tratamiento.component.html'
})
export class MdlZonaTratamientoComponent implements OnInit, OnDestroy {

    @Input() data: ZonaTratamiento | null = null;
    ...
    constructor(
        ...
        private api: ZonaTratamientoService,
    ...
    onSubmit(): void {
        ...
        if (this.data ){
            // EDITAR
          Swal.fire({
            html: `Desea editar los datos del tratamiento <b>${this.data.nombre}</b>??`,
            icon: 'question',
              ...
              if(result.isConfirmed) {
                ...
                this.api.modificar(this.model).subscribe((res: boolean | ErrorSistema)=> {
                    ...
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        html: `Se modifico los datos del tratamiento <b>${this.data.nombre}</b> con exito!!!`,
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: "Aceptar",
                        customClass: {
                  ...
                  error => {
                    Swal.fire({
                      title: 'Error',
                      html: `Error al intentar modificar los datos del tratamiento <b>${this.data.nombre}</b>`,
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
                    ...
                    console.log('Error al modificar el tratamiento', error);
                    this.loading = false;
                    this.spinner.hide();
                  });
              }else{
            ...
            // NUEVO
          Swal.fire({
            title: 'Desea registrar el nuevo tratamiento?',
            icon: 'question',
            allowOutsideClick: false,
            allowEscapeKey: false,
            confirmButtonText: 'Si',
              ...
              if(result.isConfirmed) {
                ...
                this.api.registrar(this.model).subscribe((res: boolean | ErrorSistema) => {
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        text: "Se registro el tratamiento con exito",
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: "Aceptar",
                        customClass: {
                  ...
                  error => {
                    this.loading = false;
                    console.log('Error al registrar el tratamiento', error);
                    Swal.fire({
                      title: 'Error',
                      text: 'Error al registrar el tratamiento',
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
