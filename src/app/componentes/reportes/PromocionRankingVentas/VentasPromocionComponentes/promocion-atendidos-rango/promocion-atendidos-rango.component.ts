...
@Component({
  selector: 'app-promocion-atendidos-rango',
  templateUrl: './promocion-atendidos-rango.component.html',
  styleUrls: ['./promocion-atendidos-rango.component.scss']
})

export class PromocionAtendidosRangoComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    if(this.FechaInicio && this.FechaFin) {
      ...
      this.dtResponsiveOptions = {
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.subscription = this.api.obtenerRankingAtendido(this.FechaInicio, this.FechaFin, this.IdSede, this.IdPromocion).subscribe((res) => {
            callback({data: res});
            this.collection = res;
          }, error => {
            callback({data: []});
        ...
        buttons: [
          {
            extend: 'excelHtml5',
            title: () => {
              return 'Reporte promoción ranking atendidos por sede, promoción: ' + this.promocionSeleccionada + ' - desde ' + this.FechaInicio + ' hasta ' + this.FechaFin;
            },
            exportOptions: {
              columns: [ 1,2,3 ]
            },
  ...
  buildGraph(): void{
    ...
    am4core.ready(() => {
      ...
      setTimeout( () => {
        ...
        this.chart = am4core.create("chartPromocionAtendidosRankingTotal", am4charts.XYChart);
  ...
  buildPieGraph(): void{
    ...
    am4core.ready(() => {
      ...
      // Create chart instance
      this.piechart = am4core.create("chartPromocionAtendidosRankingTotalSede", am4charts.PieChart);
