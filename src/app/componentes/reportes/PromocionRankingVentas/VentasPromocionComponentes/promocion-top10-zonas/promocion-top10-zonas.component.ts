...
@Component({
  selector: 'app-promocion-top10-zonas',
  templateUrl: './promocion-top10-zonas.component.html',
  styleUrls: ['./promocion-top10-zonas.component.scss']
})
export class PromocionTop10ZonasComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    if(this.FechaInicio && this.FechaFin) {
      ...
      this.dtResponsiveOptions = {
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.subscription = this.api.obtenerTop10Zona(this.FechaInicio, this.FechaFin, this.IdSede, 1, this.IdPromocion).subscribe((res) => {
            callback({data: res});
            this.collection = res;
            // console.log(res);
          }, error => {
        ...
        buttons: [
          {
            extend: 'excelHtml5',
            title: () => {
              return 'Reporte top 10 zonas mas agendadas con relación a la promoción: ' + this.promocionSeleccionada + ' - desde ' + this.FechaInicio + ' hasta ' + this.FechaFin;
            },
            exportOptions: {
              columns: [ 1,2,3 ]
            },
