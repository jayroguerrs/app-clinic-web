...
@Component({
  selector: 'app-top10-zonas-atendidas',
  templateUrl: './top10-zonas-atendidas.component.html',
  styleUrls: ['./top10-zonas-atendidas.component.scss']
})
export class Top10ZonasAtendidasComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    if(this.FechaInicio && this.FechaFin) {
      ...
      this.dtResponsiveOptions = {
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.subscription = this.api.ObtenerTop10Atendidas(this.FechaInicio, this.FechaFin, this.IdSede, this.IdGenero, this.numeroSesion.value, 1).subscribe((res: Zona[]) => {
            callback({data: res});
            this.collection = res;
          }, error => {
            callback({data: []});
  ...
  getExportFileName(): string{
    return `Reporte top 10 zonas mas atendidas desde ${this.FechaInicio} hasta ${this.FechaFin} (Sede - ${this.Sede}) (Genero - ${this.Genero}) (N° Sesión - ${parseInt(this.numeroSesion.value) ? this.numeroSesion.value : 'Todos' })`;
  }
