...
export class ZonasAtendidasComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    if(this.FechaInicio && this.FechaFin) {
      ...
      this.dtResponsiveOptions = {
        ajax: (dataTablesParameters: any, callback) => {
          ...
          this.subscription = this.api.ObtenerCantidadAtendidas(this.FechaInicio, this.FechaFin, this.IdSede, this.IdGenero, this.numeroSesion.value,1).subscribe((res: Zona[]) => {
            console.log(res);
            callback({data: res});
            this.collection = res;
          }, error => {
  ...
  reloadTable(): void{
    // console.log(parseInt(this.numeroSesion.value,10));
    if(this.dataTable){
  ...
  getExportFileName(): string{
    return `Reporte zonas atendidas desde ${this.FechaInicio} hasta ${this.FechaFin} (Sede - ${this.Sede}) (Genero - ${this.Genero}) (N° Sesión - ${parseInt(this.numeroSesion.value) ? this.numeroSesion.value : 'Todos' })`;
  }
