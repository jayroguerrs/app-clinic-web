...
@Component({
  selector: 'app-citas-agendadas-cortesia',
  templateUrl: './citas-agendadas-cortesia.component.html',
  styleUrls: ['./citas-agendadas-cortesia.component.scss']
})
export class CitasAgendadasCortesiaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadData(): void{
    ...
    this.subscription = this.api.ObtenerDeCortesiaRangoFechaVenta(this.FechaInicio, this.FechaFin, this.IdSede, this.IdGenero).subscribe((res) => {
      this.collection = res;
    }, error => {
      this.collection = [];
      console.log(error);
  ...
  buildGraph(): void{
    ...
    am4core.ready(() => {
      ...
      setTimeout( () => {
        ...
        this.chart = am4core.create("chartCitasAgendadasCortesiaTotal", am4charts.XYChart);
  ...
  buildPieGraph(): void{
    ...
    am4core.ready(() => {
      ...
      // Create chart instance
      this.piechart = am4core.create("chartCitasAgendadasCortesia", am4charts.PieChart);
  ...
  exportarExcel(): void{
    ...
    this.auditoriaService.insAuditoria(param).subscribe((res)=>{
      if(res.status === 200){
        const title = 'Citas agendadas - Cortesia';
        const header = ["Sede", "N° Citas", "Fecha"];

        const workbook = new Excel.Workbook();
        const worksheet = workbook.addWorksheet('Citas agendadas - Cortesia');
  ...
  getExportFileName(): string{
    return `Citas agendadas de cortesia por sede desde ${this.FechaInicio} hasta ${this.FechaFin} (Genero - ${this.Genero})`;
  }
