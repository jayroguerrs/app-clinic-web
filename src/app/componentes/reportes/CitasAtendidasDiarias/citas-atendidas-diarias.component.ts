...
import {TblReporteCitasAdendidasDiariasComponent} from "./TblReporteCitasAtendidasDiarias/tbl-reporte-citas-adendidas-diarias.component";
...
@Component({
  templateUrl: 'citas-atendidas-diarias.component.html' ,
  styleUrls: ['./citas-atendidas-diarias.component.scss'],
  providers: [DatePipe]
})
export class CitasAtendidasDiariasComponent implements OnInit, OnDestroy, AfterViewInit{
    ...
    collectionData: ReporteAtendidasDiarias[] = [];
    submitted = false;

    // Spinner
    rutaImageSpinner = '<img src="assets/images/logo-animado.gif" />';
    @ViewChild('tabla') tabla : TblReporteCitasAdendidasDiariasComponent;
    ...
    constructor(
      ...
      this.formGroup = this.formBuilder.group({
        fecha: new FormControl(this.datePipe.transform(this.today,'yyyy-MM-dd'), Validators.required),
    ...
    ngAfterViewInit(): void {
      this.tabla._collection.subscribe((res: ReporteAtendidasDiarias[]) => {
    ...
    obtenerReporte(event: string | null = null): void{
      ...
      if( this.formGroup.invalid ){
        this.utilsService.mostrarToast('Seleccionar fecha','error');
        return;
      }
  ...
  mostrarServicioNombre(id: number): string{
    const sede = this.servicios.find( s => s.id === id );
    if( sede ){
      return sede.nombre;
    }else{
      return '';
