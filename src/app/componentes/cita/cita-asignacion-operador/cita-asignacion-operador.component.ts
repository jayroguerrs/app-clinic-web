...
@Component({
  selector: 'app-cita-asignacion-operador',
  templateUrl: './cita-asignacion-operador.component.html',
  styleUrls: ['./cita-asignacion-operador.component.scss'],
  providers: [DatePipe]
})
export class CitaAsignacionOperadorComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  buildtableCita(): void{
    this.dtResponsiveOptionsCita = {
      ajax: (_dataTablesParameters: any, callback) => {
        const tipoCliente = parseInt( this.frmOperadores.controls.tipoCliente.value, 10 );
        ...
        if(this.reloadBD){
            if(this.idUsuarioReasignacion == 0) {
              this.titulo = 'Asignación de citas';
              const sinAsignar = ($('#chkSinAsignar')[0] as HTMLInputElement).checked;
              this.sbcAsignadaObtener = this.citaAsignadaService.obtener(fecha, sinAsignar, idSede, tipoCliente).subscribe(
                data => {
              ...
              this.titulo = 'Re-asignación de citas de: ' + this.nombreReasignado;
              ...
              this.sbcObtenerReasignadoByUsuario = this.citaAsignadaService.obtenerReasignadoByIdUsuario(idTipoModo, fechaConfirmacion, this.idUsuarioReasignacion).subscribe(
                data => {
                  data.forEach((a: any)  => { a.seleccionado = false });
                  this.maestroCitas = data;
                  callback({ data: this.maestroCitas });
