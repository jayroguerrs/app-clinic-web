...
@Component({
  selector: 'app-usuario-seleccion-especialista',
  templateUrl: './usuario-seleccion-especialista.component.html',
  styleUrls: ['./usuario-seleccion-especialista.component.scss']
})
export class UsuarioSeleccionEspecialistaComponent implements OnInit, AfterViewInit {
  ...
  buildtable(): void{
    ...
    this.spinner.show();
    this.usuarioService.obtenerToCita().subscribe(
      data => {
        this.userList = data;
  ...
  seleccionarUsuario(usuario): void {
    $('[data-id]').removeClass('selected');
    $('[data-id= ' + usuario.idUsuario +']').addClass('selected');
  }
