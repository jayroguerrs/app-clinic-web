...
import {TblClienteCronogramasComponent} from "../../../corporal360/componente/tables/tbl-cliente-cronogramas/tbl-cliente-cronogramas.component";
...
@Component({
  selector: 'app-cliente-perfil-corporal360',
  templateUrl: './cliente-perfil-corporal360.component.html',
  styleUrls: ['./cliente-perfil-corporal360.component.scss']
})
export class ClientePerfilCorporal360Component implements OnInit, OnDestroy {

  citasResumen: any = [];
  subscriptionCitas: Subscription;
  loadingCitas = false;
  dtOptionsTableCitasPendientes: any = {};
  dtOptionsTableCitasPendientesHollywoodPeel: any = {};
  dtOptionsTableCitasRealizadas: any = {};
  dtOptionsTableCitasRealizadasHollywoodPeel: any = {};
  ...
  @ViewChild('tblClienteCronogramas') tblClienteCronogramas: TblClienteCronogramasComponent;
  ...
  ngOnInit(): void {
    ...
    this.cargarCitasPendientesHollywoodPeel();

    this.cargarCitasRealizadas();
    this.cargarCitasRealizadasHollywoodPeel();
  ...
  reload(): void{
    ...
    this.tblClienteCronogramas.reload();
    this.documentosEmitidos?.reload();
    this.documentosEmitidosAnulados?.reload();
    this.contratos?.reload();
  }
  ...
  visualizarDocumento(): void{
    this.spinner.show();
    this.clienteDocumentoService.getDocumentById(this.documentoSelected.id).subscribe((res: DocumentoCLiente | ErrorSistema) => {
  ...
  async renderDocumento( clienteDocumento: DocumentoCLiente, enviarCorreo: boolean = false ): Promise<void> {
    ...
    if( plantilla.includes('@promocion') && plantilla.includes('@tblzonas') ){
      ...
      // console.log('data',this.maestroPromocion, promocion,clienteDocumento);
  ...
  cargarCitasPendientes(): void{
    ...
    this.dtOptionsTableCitasPendientes = {
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.subscriptionCitas = this.citaService.obtenerParaPerfilByServicio(this.idCliente, 3).subscribe(
          (resultado: CitaClass[]) => {
      ...
      columns: [
        ...
        { title: 'Cronograma', data: 'idCronograma', width: '100px', render: (data,type,row: CitaClass) => {
            const url = this.route.createUrlTree(['Corporal360/Cronograma',data,AccionCronograma.ASIGNARCITAS, this.idCliente, 0, 0, 0]);
            return data ? `<a href="${url}" target="_blank">CR-${data.toString().padStart(5,'0')}</a>` : '';
        }},
  ...
  cargarCitasPendientesHollywoodPeel(): void{

    this.dtOptionsTableCitasPendientesHollywoodPeel = {
      ajax: (dataTablesParameters: any, callback) => {
        //  Obtiene el listado de citas del cliente
        this.loadingCitas = true;

        this.subscriptionCitas = this.citaService.obtenerParaPerfilByServicio(this.idCliente, 13).subscribe(
          (resultado: CitaClass[]) => {
  ...
  cargarCitasRealizadasHollywoodPeel(): void{
    this.dtOptionsTableCitasRealizadasHollywoodPeel = {
      ajax: (dataTablesParameters: any, callback) => {
        //  Obtiene el listado de citas del cliente
        this.loadingCitas = true;
  ...
  verContrato(): void{
    const contrato = this.contratoSelected;
    this.spinner.show();
    this.subcriptionBuscarContrato = this.clienteContratoService.buscar(contrato).subscribe((contrato: ClienteContrato | ErrorSistema) => {
  ...
  irCita(estadoCita: AccionCita): void {
    window.open(`/Corporal360/Cronograma/${this.citaSelected?.idCronograma}/${AccionCronograma.ASIGNARCITAS}/${this.idCliente}/${this.citaSelected?.idPreferente}/${this.citaSelected?.id}/${estadoCita}`, '_blank');
  }
  citaVisualizar(): void {
  ...
  verCronograma(): void{
    const url = this.route.createUrlTree(['Corporal360/Cronograma',this.citaSelected.idCronograma,AccionCronograma.ASIGNARCITAS, this.idCliente, this.citaSelected?.idPreferente, 0, 0]);
