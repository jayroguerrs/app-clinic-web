...
@Component({
  selector: 'app-cliente-perfil-rejuvenecimiento-facial',
  templateUrl: './cliente-perfil-rejuvenecimiento-facial.component.html',
  styleUrls: ['./cliente-perfil-rejuvenecimiento-facial.component.scss']
})
export class ClientePerfilRejuvenecimientoFacialComponent implements OnInit, OnDestroy {
  ...
  cargarCitasPendientes(): void{
    ...
    this.dtOptionsTableCitasPendientes = {
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.subscriptionCitas = this.citaService.obtenerParaPerfilByServicio(this.idCliente, 4).subscribe(
          (resultado: CitaClass[]) => {
      ...
      columns: [
        ...
        { title: 'Cronograma', data: 'idCronograma', width: '100px', render: (data,type,row: CitaClass) => {
            const url = this.route.createUrlTree(['Corporal360/Cronograma',data,AccionCronograma.ASIGNARCITAS, this.idCliente, 0, 0]);
  ...
  irCita(estadoCita: AccionCita): void {
    window.open(`/Cita/${this.idCita}/${estadoCita}/${this.idCliente}/${this.citaSelected?.idPreferente}`, '_blank');
  }
  citaVisualizar(): void {
