...
@Component({
  selector: 'app-cliente-perfil-global',
  templateUrl: './cliente-perfil-global.component.html',
  styleUrls: ['./cliente-perfil-global.component.scss']
})
export class ClientePerfilGlobalComponent implements OnInit  {
  ...
  @ViewChild('mdlOpcionesContratos') mdlOpcionesContratos: any;
  ...
  @ViewChild('tablaCitas') tablaCitasRef: TablaClientePerfilGlobalComponent;
  subscriptionRestoreDocument: Subscription;

  modalDocumentoDatosRef: NgbModalRef;
  @ViewChild('financiamientoCuotas', { static: true }) financiamientoCuotas!: TemplateRef<any>;
  @ViewChild('historiaClinicaModal', { static: true }) historiaClinicaModal!: TemplateRef<any>;
  @ViewChild('resumenContratosModal', { static: true }) resumenContratosModal!: TemplateRef<any>;
  @ViewChild('evTratamientoModal', { static: true }) evTratamientoModal!: TemplateRef<any>;
  @ViewChild('documentosEmitidosModal', { static: true }) documentosEmitidosModal!: TemplateRef<any>;
  @ViewChild('documentosEmitidosAnuladosModal', { static: true }) documentosEmitidosAnuladosModal!: TemplateRef<any>;
  @ViewChild('historialSeguimientoCitaModal', { static: true }) historialSeguimientoCitaModal!: TemplateRef<any>;
  ...
  abrirModalZona(event: any){
    this.citaSelected = event;
    this.mostrarModalOpciones(this.mdlOpcionesCitas, event);
  }

  getFinanciamientoCuotasTemplate(): TemplateRef<any> {
    return this.financiamientoCuotas;
  }

  getHistoriaClinicaTemplate(): TemplateRef<any> {
    return this.historiaClinicaModal;
  }
  
  getResumenContratosTemplate(): TemplateRef<any> {
    return this.resumenContratosModal;
  }
  
  getEvolucionTratamientoTemplate(): TemplateRef<any> {
    return this.evTratamientoModal;
  }
  
  getDocumentosEmitidosTemplate(): TemplateRef<any> {
    return this.documentosEmitidosModal;
  }
  
  getDocumentosAnuladosTemplate(): TemplateRef<any> {
    return this.documentosEmitidosAnuladosModal;
  }
  
  getHistorialSeguimientoCitaTemplate(): TemplateRef<any> {
    return this.historialSeguimientoCitaModal;
  }
  
  // Funcion Menú Escritorio
  abrirModales(modal): void{
    this.modalDocumentoDatosRef = this.utilService.abrirModal(modal,'xl');
  ...
  cargarHistorialSeguimiento(): void{
    ...
    this.subscriptionHistorial = this.citaSeguimientoService.obtenerHistorialSeguimientoPorServicio(this.idCliente, this.idServicio).subscribe(
      resultado => {
        this.historiales  = resultado;
        this.subscriptionZonasAtendidas = this.clienteService.obtenerTodasZonasAtendidasPorServicio(this.idCliente, this.idServicio).subscribe(
          resultado => {
  ...
  mostrarHistoriaClinicaOpciones(modal, historiaClinica: HistoriaClinicaCliente | null = null): void {
    ...
    // modal.show();
    this.modalService.open(this.mdlOpcionesHistoria);
  }
  anularHistoria(): void{
  ...
  mostrarContratoOpciones(modal, contrato: ClienteContrato | null = null): void {
    ...
    // modal.show();
    this.modalService.open(this.mdlOpcionesContratos);
  }
  ...
  mostrarEvolucionTratamientOpciones(modal, compEvTratamiento: EvolucionTratamiento | null = null): void {
    ...
    // modal.show();
    this.modalService.open(this.mdlOpcionesEvolucion);
  }
  ...
  citaVisualizarTratamiento(idCita: number, idEvoclucion: number): void {
    this.irCitaTratamiento(AccionCita.VER, idCita, idEvoclucion);
  }

  irCitaTratamiento(estadoCita: AccionCita, idCita: number, idEvoclucion: number): void {
    window.open(`/Cita/${idCita}/${estadoCita}/${this.idCliente}/0/${this.idServicio}?Mdltratamientoid=${idEvoclucion}`, '_blank');
  }
  ...
  editarEvolucionTratamiento(modal: any): void{
    ...
    this.citaVisualizarTratamiento(this.evolucionTratamientoSelected.idCita, this.evolucionTratamientoSelected.id);
  ...
  async renderDocumento( clienteDocumento: DocumentoCLiente, enviarCorreo: boolean = false ): Promise<void> {
    ...
    if(plantilla.includes('@patologia')){
      parametros['patologia'] = clienteDocumento.patologias.map(x => x.nombre).join(", ");
