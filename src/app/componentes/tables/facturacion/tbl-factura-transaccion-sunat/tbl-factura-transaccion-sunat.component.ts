...
@Component({
  selector: 'tbl-factura-transaccion-sunat',
  templateUrl: './tbl-factura-transaccion-sunat.component.html',
  styleUrls: ['./tbl-factura-transaccion-sunat.component.scss']
})
export class TblFacturaTransaccionSunatComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  selected = new Subject<FacturaTransaccionSunat | null>();
  dataTable: any;
  ...
  initValues(): void{
    this.optionDataTable = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.subscription = this.api.listar( this.auth.getUser().id ).subscribe((data: FacturaTransaccionSunat[]) => {
          ...
          console.log('Error al obtener los tipos de transacción', err);
          callback([]);
          this.utilService.mostrarToast('Error al obtener los tipos de transacción','warning');
        }, () => {
          this.loading = false;
        });
