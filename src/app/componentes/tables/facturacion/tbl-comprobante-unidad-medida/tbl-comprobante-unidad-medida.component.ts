...
@Component({
  selector: 'tbl-comprobante-unidad-medida',
  templateUrl: './tbl-comprobante-unidad-medida.component.html',
  styleUrls: ['./tbl-comprobante-unidad-medida.component.scss']
})
export class TblComprobanteUnidadMedidaComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  selected = new Subject<ComprobanteUnidadMedida | null>();
  dataTable: any;

  constructor(
    private api: ComprobanteUnidadMedidaService,
  ...
  initValues(): void{
    this.optionDataTable = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.subscription = this.api.listar( this.auth.getUser().id ).subscribe((data: ComprobanteUnidadMedida[]) => {
          ...
          console.log('Error al obtener las unidades de medida', err);
          callback([]);
          this.utilService.mostrarToast('Error al obtener las unidades de medida','warning');
        }, () => {
          this.loading = false;
        });
