...
@Component({
  selector: 'tbl-factura-token',
  templateUrl: './tbl-factura-token.component.html',
  styleUrls: ['./tbl-factura-token.component.scss']
})
export class TblFacturaTokenComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  selected = new Subject<FacturaToken | null>();
  dataTable: any;
  ...
  initValues(): void{
    this.optionDataTable = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.subscription = this.api.listar( this.auth.getUser().id ).subscribe((data: FacturaToken[]) => {
          ...
          console.log('Error al obtener los tokens de facturación', err);
          callback([]);
          this.utilService.mostrarToast('Error al obtener los tokens de facturación','warning');
        }, () => {
          this.loading = false;
        });
      ...
      columns: [
        ...
        { title: 'Token', data: 'token', class: 'fw-bold' },
