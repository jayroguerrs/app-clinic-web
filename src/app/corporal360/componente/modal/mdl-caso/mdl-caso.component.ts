...
@Component({
    selector: 'app-mdl-caso',
    templateUrl: 'mdl-caso.component.html'
})
export class MdlCasoComponent implements OnInit, OnDestroy {

    @Input() data: Caso | null = null;
    ...
    onSubmit(): void {
        ...
        if (this.data ){
            // EDITAR
          Swal.fire({
            html: `Desea editar los datos del caso <b>${this.data.nombre}</b>??`,
            icon: 'question',
              ...
              if(result.isConfirmed) {
                ...
                this.api.modificar(this.model).subscribe((res: boolean | ErrorSistema)=> {
                    ...
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        html: `Se modifico los datos del caso <b>${this.data.nombre}</b> con exito!!!`,
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: "Aceptar",
                        customClass: {
                  ...
                  error => {
                    Swal.fire({
                      title: 'Error',
                      html: `Error al intentar modificar los datos del caso <b>${this.data.nombre}</b>`,
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
                    ...
                    console.log('Error al modificar el caso', error);
                    this.loading = false;
                    this.spinner.hide();
                  });
              }else{
            ...
            // NUEVO
          Swal.fire({
            title: 'Desea registrar el nuevo caso??',
            icon: 'question',
            allowOutsideClick: false,
            allowEscapeKey: false,
            confirmButtonText: 'Si',
              ...
              if(result.isConfirmed) {
                ...
                this.api.registrar(this.model).subscribe((res: boolean | ErrorSistema) => {
                    if (res instanceof ErrorSistema){
                      ...
                      Swal.fire({
                        text: "Se registro el caso con exito",
                        icon: "success",
                        buttonsStyling: false,
                        confirmButtonText: "Aceptar",
                        customClass: {
                  ...
                  error => {
                    ...
                    Swal.fire({
                      title: 'Error',
                      text: 'Error al registrar el caso',
                      icon: 'error',
                      buttonsStyling: false,
                      confirmButtonText: 'Aceptar',
                      customClass: {
