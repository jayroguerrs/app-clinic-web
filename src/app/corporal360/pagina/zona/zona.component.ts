...
import {Zona, ZonaServicio} from '../../shared/model/zona';
import { MdlZonaComponent } from '../../componente/modal/mdl-zona/mdl-zona.component';
...
export class ZonaComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  buildtable(): void{
    this.dtResponsiveOptions = {
      ...
      columns: [
        ...
        { title: 'GENERO', data: 'genero', className: 'text-uppercase', width: '100px'},

        { title: 'SERVICIOS', data: 'servicios', className: 'text-uppercase', width: '20px', render: (data: ZonaServicio[]) => {
          let output = '';
          data.forEach(x => {
            output += `<div class="rounded d-inline-block border border-primary mx-1 bg-primary"><span class="p-1 d-block text-center font-weight-bold">${x.nombreCorto}</span ><span class="p-1 d-block text-center">${x.duracion}</span></div>`;
          });
          return output;
        }},

        { title: 'U. REGISTRO', data: 'usuarioRegistro', width: '150px' },
        { title: 'F. REGISTRO', data: 'fechaRegistro', width: '100px', render: (data: Date) =>{
            return this.datePipe.transform(data, 'dd-MM-yyyy')
        }},
        { title: 'U. MODIFICO', data: 'usuarioModifico', width: '150px'},
  ...
  onCreate(): void{
    const modalRef = this.modalService.open(MdlZonaComponent);
