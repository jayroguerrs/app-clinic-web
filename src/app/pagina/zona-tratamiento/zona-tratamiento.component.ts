...
import {MdlZonaTratamientoComponent} from "../../componentes/modals/mdl-zona-tratamiento/mdl-zona-tratamiento.component";
...
@Component({
  selector: 'app-zona-tratamiento',
  templateUrl: './zona-tratamiento.component.html',
  styleUrls: ['./zona-tratamiento.component.scss']
})
export class ZonaTratamientoComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  buildtable(): void{
    this.dtResponsiveOptions = {
      ajax: (dataTablesParameters: any, callback) => {
        this.spinner.show();
        this.sbcCollection = this.api.collection(this.auth.getUser().id).subscribe(
          resultado => {
  ...
  onCreate(): void{
    const modalRef = this.modalService.open(MdlZonaTratamientoComponent, {size: ' max-w-500px mx-auto', windowClass: 'smodal round popins bg-dark-30', keyboard: false, centered: true,  animation: true, backdrop: "static" });
