...
@Component({
  selector: 'app-comprobante-electronico-reporte-pago',
  templateUrl: './comprobante-electronico-reporte-pago.component.html',
  styleUrls: ['./comprobante-electronico-reporte-pago.component.scss']
})
export class ComprobanteElectronicoReportePagoComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  collection: FacturacionReportePago[] = [];
  selected: FacturacionReportePago | null = null;
    ...
    this.dtResponsiveOptions = {
      ajax: (dataTablesParameters: any, callback) => {
        ...
        if(!this.submitted){
          ...
          this.subscription = this.api.obtenerReportePago(this.usuarioActual.id, idSede, idTipoComprobante, fechaDesde, fechaHasta).subscribe( (res: FacturacionReportePago[] | ErrorSistema) => {
      ...
      columns: [
        ...
        { title: 'Días', data: 'dias', visible: true},
        { title: 'Propina', data: 'propina', visible: true},
        { title: 'Total', data: 'total', visible: true, render: (data: number) => {
            return data.toFixed(2);
          }},
        { title: 'FPAGO', data: 'idTipoPago', visible: true},
        { title: 'Descripcion', data: 'tipoPago', visible: true},
        { title: 'NRO OPERACIÓN', data: 'numeroOperacion', visible: true},
        { title: 'E', data: 'e', visible: true},
        { title: 'Obs. FormaPago', data: 'observacionFormaPago', visible: true},
        { title: 'CÓDIGO', data: 'documentoCliente', visible: true},
        { title: 'CLIENTE', data: 'nombreCliente', visible: true},
        { title: 'USUARIO', data: 'usuario', visible: true},
        { title: 'CAJA', data: 'caja', visible: true},
        { title: 'TURNO', data: 'turno', visible: true},
        { title: 'Obs.Comprobante', data: 'observacionComprobante', visible: true},
      ],
      serverSide: false,
      processing: false,
      async: true,
      buttons: [
        {
          extend: 'excelHtml5',
          title: () => {
            return `Reporte de pagos del ${this.fechaDesde} al ${this.fechaHasta}`;
          },
          autoFilter: true,
          sheetName: 'Data',
          exportOptions: {
