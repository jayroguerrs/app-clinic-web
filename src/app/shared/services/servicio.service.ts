...
export class ServicioService {
    ...
    listar(): Observable<Servicio[]> {
        return this.http.get<any>(`${environment.apiUrl}/api/servicio`, {headers: this.headers}).pipe(
          map((res) =>{
  ...
  listarByEstado(idEstado: number): Observable<Servicio[]> {
    return this.http.get<any>(`${environment.apiUrl}/api/servicio/estado/${idEstado}`, {headers: this.headers}).pipe(
      map((res) =>{
        const collection: Servicio[] = [];
        if(res.status === 200){
          res.data.forEach((item) => {
    ...
    registrar(servicio: Servicio): Observable<boolean | ErrorSistema >{
      return this.http.post(`${environment.apiUrl}/api/servicio`, servicio, {headers: this.headers}).pipe(
        map((res: any) =>{
          if(res.status === 201 ){
            return true;
          }else{
    ...
    modificar(servicio: Servicio): Observable<boolean | ErrorSistema >{
      return this.http.put(`${environment.apiUrl}/api/servicio/${servicio.id}`, servicio, {headers: this.headers}).pipe(
        map((res: any) =>{
          if(res.status === 200 ){
            return true;
          }else{
